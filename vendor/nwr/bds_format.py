"""Read, validate, and atomically write Neural World Runtime .bds files."""
...
def _anchor(**values: float) -> list[float]:
    channels = [name for group in CHANNEL_SCHEMA.values() for name in group]
...
ANCHORS: dict[str, list[float]] = {
    ...
    # A wall that deflects flow without claiming human authority. Video
    # contours and AI/entity barriers use this so only a human can mint a lock.
    "solid": _anchor(
        density=0.4,
        albedo_r=0.3,
        albedo_g=0.34,
...
def create_initial_grid(
    ...
    """Create a deterministic, unlocked seed suitable for the playground."""
    grid = create_empty_grid(width, height)
    yy, xx = np.ogrid[:height, :width]
    cx, cy = width / 2.0, height / 2.0
    core = (xx - cx) ** 2 + (yy - cy) ** 2 <= (min(width, height) * 0.11) ** 2
    halo = (
        ((xx - cx) ** 2 + (yy - cy) ** 2 <= (min(width, height) * 0.19) ** 2)
        & ~core
    )
    fluid = np.asarray(ANCHORS["active_fluid"], dtype=DTYPE)
    grid[core] = fluid
    grid[halo] = fluid * np.float32(0.35)
    grid[..., 31] = 0.0
...
def make_header(
    ...
    *,
    ...
    Unknown optional fields such as ``ai_metadata`` are ignored by older
    readers: validation only enforces the required structural keys.
    """
    ...
    if include_ai_metadata:
        from ai_world import build_ai_metadata

        # Compact form: build-wide catalogs stay in the runtime so the 4 KiB
        # header budget is spent on this world, not on constants.
        header["ai_metadata"] = build_ai_metadata(
...
def _validate_header(header: Any) -> None:
    ...
    if (
        ...
        raise BDSFormatError("grid_dimensions must be [positive width, positive height, 1]")
    ...
    expected_payload = dimensions[0] * dimensions[1] * VECTOR_DIMENSIONS * DTYPE.itemsize
    if (
