...
```
Host agent (PrismAPI / Website Hub)
    → POST /prism/speak   (AI↔AI)
...
| Layer | Role |
| --- | --- |
| **PrismAPI / `/prism/speak`** | AI-to-AI control — agents send assistant text |
| **FaceBridge** | Internal GPU control plane inside the container |
| **MJPEG `/stream.mjpg`** | Browser embed (no WebGL port required) |

Integrators should use **PrismAPI-shaped speak** (or `POST /prism/speak`), not
ad-hoc browser calls to a desktop window.
...
forward_speak({"text": "Hello there"})           # → FaceBridge
...
4. Hub already fire-and-forgets speak after assistant replies.
...
2. `POST /prism/speak` moves the mouth
3. Browser shows live face at `/stream.mjpg?token=…`
4. Website chat still works if the face container is down
