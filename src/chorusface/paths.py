...
Everything ChorusFace *writes* goes under ``output/`` in the current working
directory, or under ``$CHORUSFACE_OUTPUT`` when that is set. Nothing is written
...
name. :func:`chorusface.parts.default_parts_path` relies on that contract::
...
OUTPUT: Final = Path(os.environ.get("CHORUSFACE_OUTPUT", "output")).resolve()
