...
from chorusface.runtime.shaders import (
    COMPUTE_PASSES,
    WORKGROUP_SIZE,
    load_compute_passes,
    load_shader,
...
def encode_jpeg(
    ...
    *,
    ...
    """Encode a bottom-up OpenGL pixel buffer as a top-down JPEG (web stream)."""
    from PIL import Image
...
class FieldRuntime(mglw.WindowConfig):
    ...
    def _load_startup_world(self) -> np.ndarray:
        if not self.world_path.exists():
            raise FileNotFoundError(
                f"World not found: {self.world_path}. Build one with `chorusface-seed`."
            )
    ...
    def _read_preview_rgb(self) -> bytes:
        """Render the live face at 1024x1024 (portrait only) → raw RGB bytes."""
    ...
    def _preview(self) -> bytes:
        ...
        """
        return encode_png(
            self._read_preview_rgb(), PREVIEW_RESOLUTION, PREVIEW_RESOLUTION, 3
        )

    def _preview_jpeg(self) -> bytes:
        """Same portrait preview as PNG, encoded JPEG for MJPEG web embed."""
        return encode_jpeg(
            self._read_preview_rgb(),
            PREVIEW_RESOLUTION,
            PREVIEW_RESOLUTION,
            3,
            quality=82,
