...
from chorusface.tickfeed.driver import face_box_from_profile
from chorusface.tickfeed.package import FaceBox
from chorusface.tickfeed.schema import TICK_RATE_HZ
...
# Farneback on articulation. Override with CHORUSFACE_TICKFEED_FLOW=farneback.
_FLOW_METHOD = os.environ.get("CHORUSFACE_TICKFEED_FLOW", "dis").strip().lower()
...
def prepare_face_timeline(
    ...
    *,
    ...
    if flow_series is not None:
        ...
        if vid is not None and vid.is_file():
            try:
                from chorusface.tickfeed.lid_measure import measure_lid_series
