...
from chorusface.tickfeed.calibration import beat_at_time, load_calibration_script
from chorusface.tickfeed.chorus_transport import TickFeedTransport
from chorusface.tickfeed.cosmetics import CosmeticPrefs, load_cosmetic_prefs
...
from chorusface.tickfeed.schema import (
    KEY_REFRESH_TICKS,
    TICK_RATE_HZ,
    BeatId,
    EmotionId,
...
from chorusface.tickfeed.synth import labels_from_drives, synthesize_velocity
...
class TickFeedDriver:
    ...
    def try_load_timeline(
        ...
        try:
            ...
            from chorusface.tickfeed.ml.train import CODE_DIM

            spool_all = os.environ.get("CHORUSFACE_CHORUS_SPOOL_ALL", "").lower() in (
                "1",
                "true",
                "yes",
            )
    ...
    def push_drives(
        ...
        *,
        ...
        # (CHORUSFACE_TICKFEED_ABSOLUTE=1) to diagnose Δ residue.
        absolute = os.environ.get("CHORUSFACE_TICKFEED_ABSOLUTE", "0").strip().lower() in {
            "1",
            "true",
            "yes",
            "on",
    ...
    def ingest_from_wire(self, tick: int) -> TickPackage | None:
        ...
        if source == "package":
            from chorusface.tickfeed.package import decode
