...
from chorusface.tickfeed.audio_feat import AUDIO_FEAT, load_audio_feat
from chorusface.tickfeed.calibration import load_calibration_script, beat_at_time
from chorusface.tickfeed.schema import TICK_RATE_HZ, VISEME_TABLE

CODE_DIM = 64
ML_DIR = "ml"
# Upgrade PCA → nonlinear AE when holdout reconstruction MAE exceeds this
# (NWR-scaled patches ~±1.5). Force with CHORUSFACE_TICKFEED_L4_AE=1.
L4_PCA_HOLDOUT_MAE_MAX = float(os.environ.get("CHORUSFACE_TICKFEED_L4_PCA_MAE", "0.12"))


def _force_l4_ae() -> bool:
    return os.environ.get("CHORUSFACE_TICKFEED_L4_AE", "").strip().lower() in {
        "1",
        "true",
        "yes",
        "ae",
