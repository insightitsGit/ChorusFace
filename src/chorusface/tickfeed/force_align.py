...
def _api_key() -> str:
    return (
        os.environ.get("CHORUSFACE_LLM_API_KEY")
...
def _whisper_word_spans(wav_path: Path, prompt: str) -> list[Any] | None:
    ...
    if not key:
        return None
    try:
        from chorusface.audio import decode_wav
        ...
        return None
    base_url = (
        os.environ.get("CHORUSFACE_LLM_BASE_URL")
        or os.environ.get("OPENAI_BASE_URL")
        or DEFAULT_BASE_URL
    )
    model = os.environ.get("CHORUSFACE_TTS_TRANSCRIBE_MODEL") or DEFAULT_TRANSCRIBE_MODEL
    try:
...
def force_align_speech(
    ...
    *,
    ...
    with tempfile.TemporaryDirectory(prefix="chorusface_align_") as tmp:
