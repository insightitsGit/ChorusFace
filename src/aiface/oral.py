...
``aiface-capture`` has written colocated ``smile.png`` / ``open.png``, the
...
and tooling. Runtime loading lives in ``aiface.app`` / ``avatar.frag``.
"""
...
# Written by aiface-capture; loaded by the avatar runtime when both exist.
...
def default_open_mouth_path(world: str | Path) -> Path:
    """Colocated open-mouth plate path (``open.png`` from aiface-capture)."""
    return Path(world).with_name(OPEN_MOUTH_PLATE_NAME)


def default_smile_plate_path(world: str | Path) -> Path:
    """Colocated smile plate path (``smile.png`` from aiface-capture)."""
