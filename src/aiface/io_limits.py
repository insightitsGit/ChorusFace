...
def resolve_bridge_token(
    *,
    env_var: str = "AIFACE_BRIDGE_TOKEN",
    ...
    raise RuntimeError(
        ...
        "Run scripts/run_tickfeed_demo.py or export AIFACE_BRIDGE_TOKEN."
    )
...
def require_unpinned_models_allowed() -> bool:
    ...
    flag = os.environ.get("AIFACE_REQUIRE_MODEL_DIGEST", "").strip().lower()
    if flag in {"1", "true", "yes", "on"}:
        return False
    allow = os.environ.get("AIFACE_ALLOW_UNPINNED_MODELS", "1").strip().lower()
...
def safe_joblib_load(model_path: Path | str, *, world_root: Path | str | None = None):
    ...
    if expected is not None:
        ...
        raise ValueError(
            ...
            "checksums.json, or set AIFACE_ALLOW_UNPINNED_MODELS=1 for lab"
        )
