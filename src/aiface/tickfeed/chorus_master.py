...
back to AIFace. This servicer writes each verified direct-mode vector into
...
logger = logging.getLogger("aiface.chorus_master")
...
RECV_KEEP = int(os.getenv("AIFACE_CHORUS_RECV_KEEP", "240"))
...
class MasterTargetServicer(fabric_pb2_grpc.TargetPodServicer):
    """Target Pod that persists decrypted vectors for AIFace Side A."""
    ...
    def __init__(self, spool: Path | None = None) -> None:
        ...
        logger.info("AIFace master target -> CP %s spool=%s", cp_addr, self.spool)
...
def serve(spool: Path | str | None = None) -> None:
    ...
    server.start()
    logger.info("AIFace CHORUS master target listening on %s", addr)
