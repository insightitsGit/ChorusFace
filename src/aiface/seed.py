...
``seed`` extra: ``pip install "aiface[seed]"``.

Example::

    aiface-seed --synthetic
    aiface-seed --input portrait.jpg --output output/worlds/avatar/avatar_face.bds
"""
...
from aiface.runtime.shaders import normalize_priority
...
def _cv2() -> Any:
    try:
        ...
        raise AvatarSeedError(
            ...
            '`python -m pip install "aiface[seed]"`.'
        ) from exc
    return cv2
...
def load_face_image(
    ...
    *,
    ...
    from aiface.landmarks import normalize_face_image
...
def build_avatar_seed(
    ...
    *,
    ...
    from aiface.landmarks import eye_aperture_score, measure_landmarks
...
def write_seed_bundle(
    ...
    *,
    ...
    """
    from aiface.biomechanics.muscles import load_face_definition
    from aiface.landmarks import render_seed_qa
    from aiface.parts import build_face_part_atlas, save_face_part_atlas
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    print(f"Play with: aiface --world {Path(written['world']).as_posix()}")
    return 0
