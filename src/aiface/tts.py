...
first audio sample, which :func:`aiface.speech.schedule_spans` turns into
...
def _multipart_body(
    ...
    *,
    ...
    boundary = f"----aiface{secrets.token_hex(16)}"
    parts: list[bytes] = []
...
class SapiVoice:
    ...
    def synthesize(self, text: str) -> AudioClip:
        ...
        if self.rate is not None:
            env["AIFACE_SAPI_RATE"] = str(int(self.rate))
        if self.voice:
            env["AIFACE_SAPI_VOICE"] = self.voice
        try:
            completed = subprocess.run(
                [
                    self._shell,
